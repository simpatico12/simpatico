...
st.title("📈 AI 코인 자동매매 대시보드")
st.markdown("AI 기반 자동매매 기록 및 반성 결과를 시각적으로 확인할 수 있습니다.")

# 날짜 필터
st.subheader("📅 거래 날짜 선택")
...
if df.empty:
    st.warning("❗ 선택한 날짜 범위에 거래 기록이 없습니다.")
else:
    ...
    st.subheader("📈 신뢰도 분포")
    fig_conf = px.histogram(df, x="confidence_score", nbins=10, title="신뢰도 분포")
    st.plotly_chart(fig_conf, use_container_width=True)

    st.subheader("📉 매수가 대비 현재 수익률")
    df["수익률(%)"] = ((df["coin_price"] - df["avg_buy_price"]) / df["avg_buy_price"]) * 100
    fig_profit = px.line(df.sort_values("timestamp"), x="timestamp", y="수익률(%)", color="coin", title="실현 수익률 추이")
...
if reflection_df.empty:
    st.info("🙇 아직 반성 기록이 없습니다. 추후 작성해 주세요.")
else:
    st.dataframe(reflection_df, use_container_width=True)

    st.subheader("🎯 반성 성공률 분포")
    fig_success = px.histogram(reflection_df, x="success_rate", nbins=10, title="거래 반성 성공률 분포")
    st.plotly_chart(fig_success, use_container_width=True)

st.caption("© 2025 자동매매 시스템 - Warren Buffett 전략 기반 AI 트레이딩")
