...
from utils import (
    ...
    send_telegram, is_holiday_or_weekend, get_total_asset_value, get_cash_balance
)
import schedule
import random
import time
...
def run_assets(asset_list, asset_type):
    if is_holiday_or_weekend() and asset_type != "coin":
        send_telegram(f"⏸️ {asset_type.upper()} 시장 휴장일 (주말/공휴일)")
        return
    total_asset_value = get_total_asset_value(upbit)
    ...
    for asset in asset_list:
        ...
        execute_trade(asset, asset_type, fg, sentiment, rsi, momentum, price_change,
...
if __name__ == "__main__":
    send_telegram("✅ AI 퀀트 자동매매 스케줄러 시작됨")

    # 코인 (월/금)
    schedule.every().monday.at("08:30").do(collect_news, COINS, "coin")
    schedule.every().monday.at("09:00").do(run_assets, COINS, "coin")
    schedule.every().friday.at("08:30").do(collect_news, COINS, "coin")
    schedule.every().friday.at("09:00").do(run_assets, COINS, "coin")

    # 일본 (화/목)
    schedule.every().tuesday.at("10:00").do(collect_news, JPN_STOCKS, "japan")
    schedule.every().tuesday.at("10:30").do(run_assets, JPN_STOCKS, "japan")
    schedule.every().thursday.at("10:00").do(collect_news, JPN_STOCKS, "japan")
    schedule.every().thursday.at("10:30").do(run_assets, JPN_STOCKS, "japan")

    # 미국 (화/목)
    schedule.every().tuesday.at("22:30").do(collect_news, US_STOCKS, "us")
    schedule.every().tuesday.at("23:00").do(run_assets, US_STOCKS, "us")
    schedule.every().thursday.at("22:30").do(collect_news, US_STOCKS, "us")
    schedule.every().thursday.at("23:00").do(run_assets, US_STOCKS, "us")
