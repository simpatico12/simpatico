from core.strategy_coin import analyze_coin
from core.strategy_japan import analyze_japan
from core.strategy_us import analyze_us
...
def analyze_only_coin():
    for coin in COINS:
        try:
            ...
            send_telegram(f"🔎 [코인 {coin}] 판단: {signal['decision']} | 신뢰도: {signal['confidence_score']}%\n이유: {signal['reason']}")
...
def analyze_only_japan():
    for stock in JPN_STOCKS:
        try:
            ...
            send_telegram(f"🔎 [일본 {stock}] 판단: {signal['decision']} | 신뢰도: {signal['confidence_score']}%\n이유: {signal['reason']}")
        except Exception as e:
...
def analyze_only_us():
    for stock in US_STOCKS:
        try:
            ...
            send_telegram(f"🔎 [미국 {stock}] 판단: {signal['decision']} | 신뢰도: {signal['confidence_score']}%\n이유: {signal['reason']}")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # === 테스트 스케줄 (주석 해제 시 사용 가능) ===
    # schedule.every(1).minutes.do(analyze_only_coin)
    # schedule.every(2).minutes.do(analyze_only_japan)
    # schedule.every(3).minutes.do(analyze_only_us)
